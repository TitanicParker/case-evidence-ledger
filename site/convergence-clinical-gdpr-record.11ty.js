...
module.exports = class {
  data() {
    return { permalink: "/convergence-clinical-gdpr-record/", eleventyExcludeFromCollections: true };
  }

  render() {
    const sourcePath = path.join(process.cwd(), "convergence-clinical-gdpr-record.md");
    ...
    const pageHead = head(
      "The Convergence of the Clinical Record and the GDPR Record",
      "A plain-language account of how the clinical and data-protection histories intersect while remaining legally distinct."
    ).replace("</head>", `${styles}</head>`);

    return `<!doctype html><html lang="en">${pageHead}<body class="route-home"><a class="skip-link" href="#content">Skip to content</a>${header("home")}<main id="content" class="plain-language-main"><div class="plain-language-note">Related reading · clinical record and GDPR record</div><article class="plain-language-document">${body}</article></main>${footer()}</body></html>`;
  }
};
