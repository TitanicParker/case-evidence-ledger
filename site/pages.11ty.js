...
function link(object, text = null) {
  ...
  return path ? `<a href="${esc(path)}">${esc(label)}</a>` : `<span>${esc(label)}</span>`;
}
function copy(value, extra = "") {
  return `<div class="controlled-copy ${extra}">${esc(value || "")}</div>`;
}
function objectLabel(object) { return LABELS[object.object_type] || object.object_type.replaceAll("_", " "); }
function notice() {
  return `<p class="generated-notice">Generated from the controlled repository evidence graph. This page is an interface view, not an independent evidential source.</p>`;
}
function header() {
  return `<header class="site-header"><a class="brand" href="/evidence/"><span>Case Evidence Ledger</span><small>Controlled documentary system</small></a><nav aria-label="Primary"><a aria-current="page" href="/evidence/">Evidence</a><a href="/case-evidence-ledger_timeline.html">Timeline</a><a href="/method/">Method</a><a href="https://github.com/TitanicParker/case-evidence-ledger">Repository</a></nav></header>`;
}
...
function shell(route, body, version, object = null) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} · Case Evidence Ledger</title><meta name="description" content="Canonical Evidence Foundation for the Case Evidence Ledger"><link rel="stylesheet" href="/assets/styles.css"></head><body class="${klass}"><a class="skip-link" href="#content">Skip to evidence</a>${header()}<main id="content">${body}</main><footer class="site-footer"><span>Case Evidence Ledger</span><span>Schema ${esc(version.schema_version || "1.0.0")}</span><span>Generated documentary interface</span></footer></body></html>`;
}
function identity(object) {
  return `<div class="object-kicker"><span class="object-type">${esc(objectLabel(object))}</span><code class="object-id">${esc(object.id)}</code></div>${object.title ? `<h1>${esc(object.title)}</h1>` : `<h1>${esc(object.id)}</h1>`}`;
}
...
function pageBack() { return `<a class="back-link" href="/evidence/">← Back to Evidence Foundation</a>`; }
...
function landing(ledger) {
  ...
  return `<section class="foundation-hero"><div class="eyebrow">Evidence Foundation · Schema ${esc(ledger.siteVersion.schema_version || "1.0.0")}</div><h1>Controlled Evidence Foundation</h1><p>The documentary substrate of the case, organised as Sources, Facts, Evidential Tensions and Controlled Propositions.</p><div class="hierarchy-line" aria-label="Evidence hierarchy"><span>Source</span><b>→</b><span>Fact</span><b>→</b><span>Tension</span><b>→</b><span>Proposition</span></div></section><section class="foundation-grid">${cards.map(([title,count,desc,url])=>`<a class="entry-card" href="${url}"><span class="entry-count">${count.toLocaleString("en-GB")}</span><h2>${title}</h2><p>${desc}</p><span class="entry-action">Enter collection →</span></a>`).join("")}</section><section class="foundation-secondary"><div><div class="eyebrow">Coming next</div><h2>Evidence search</h2><p>Full-text and filtered search is deliberately reserved for Gate 4. Exact evidence IDs remain navigable through the collections.</p><span class="disabled-control" aria-disabled="true">Search · Gate 4</span></div><div><div class="eyebrow">Orientation</div><h2>Timeline & method</h2><p>The existing documentary timeline remains unchanged. Method explains provenance, generated-view status and repository authority.</p><p><a href="/case-evidence-ledger_timeline.html">Open documentary timeline →</a><br><a href="/method/">Read methodology →</a></p></div></section><section class="version-strip"><span>Repository build</span><code>${esc(ledger.siteVersion.site_build_commit || "unavailable")}</code><span>${c.total.toLocaleString("en-GB")} normalized objects</span></section>`;
}
